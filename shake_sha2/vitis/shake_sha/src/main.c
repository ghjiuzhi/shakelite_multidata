...
// 模式定义
#define MODE_SHA2_256       0      // SHA2-256 模式 (4'b0000)
#define MODE_SHA2_512       1      // SHA2-512 模式 (4'b0001)
#define MODE_SHAKE_256      9      // SHAKE256 模式 (4'b1001)
...
void print_input_data(void) {
    xil_printf("Input Data (16 bytes): ");
    for (int i = 0; i < 16; i++) {
        xil_printf("%02X", input_data[i]);
    }
    xil_printf("\r\n");
}
...
//        do {
//            status = SHAKE_SHA2_IP_mReadReg(base_address, REG_STATUS);
//            xil_printf("DEBUG: Waiting tready for byte 0... Status=0x%08X\r\n", status);
//        } while ((status & STATUS_TREADY) == 0);
//
//        // 写入 tdata (低8位)
//        u32 tdata_value = input_data[0];  // 0x01
//        xil_printf("DEBUG: Write byte 0: 0x%02X\r\n", tdata_value);
//        SHAKE_SHA2_IP_mWriteReg(base_address, REG_SHA2_TDATA, tdata_value);
//
//        // 写入 tid (设为0)
//        SHAKE_SHA2_IP_mWriteReg(base_address, REG_SHA2_TID, 0);
//
//        // 设置 tvalid=1, tlast=1 (单字节传输结束)
//        u32 sha2_control = 1 | (1 << 1);  // tvalid=1, tlast=1
//        xil_printf("DEBUG: Set tvalid and tlast, SHA2 Control reg = 0x%08X\r\n", sha2_control);
//        SHAKE_SHA2_IP_mWriteReg(base_address, REG_SHA2_CONTROL, sha2_control);
//
//        // 清零 tvalid，形成脉冲
//        sha2_control = 0;
//        xil_printf("DEBUG: Clear tvalid, SHA2 Control reg = 0x%08X\r\n", sha2_control);
//        SHAKE_SHA2_IP_mWriteReg(base_address, REG_SHA2_CONTROL, sha2_control);
//
//        // 检查status
//        status = SHAKE_SHA2_IP_mReadReg(base_address, REG_STATUS);
//        xil_printf("DEBUG: After 1 byte, Status=0x%08X\r\n", status);
//
//        // 4. 等待 sha2_ovalid
...
/**
 * SHAKE256 测试函数 - 处理16字节输入，分两次8字节发送
 * @param base_address AXI基地址
 * @param result 结果存储数组（至少42个u32）
 * @return 0 如果成功，-1 如果失败
 */
int simple_shake256_test(u32 base_address, u32 *result) {
    ...
    // 6. 直接读取结果（不等待延时）
    u32 status = SHAKE_SHA2_IP_mReadReg(base_address, REG_STATUS);
