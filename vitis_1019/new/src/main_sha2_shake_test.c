/*
 * main_sha2_test.c (V3 - 修正 %llu 打印問題)
 * 用於測試 SHA-2 硬件加速
 */
#include <stdio.h>
#include <string.h>
...
// #include "params.h"      // 註釋掉，因為 SPX_ALG 宏會導致問題
#include "randombytes.h" // 消息隨機數
...
int main()
{
    ...
    // 打印當前測試的參數集 (SPX_ALG 這行已註釋掉，以避免編譯錯誤)
    ...
    XTime_GetTime(&t_end);
    hw_sign_ticks = t_end - t_start; // 暫時先用這個時間，下一步會累加
    ...
    XTime_GetTime(&t_end);
    hw_sign_ticks += (t_end - t_start); // 累加簽名時間

    // [!! 已修正 !!] 將 %llu 改為 %u，並將 (unsigned long long) 強制轉換為 (unsigned int)
    xil_printf("  Signing complete. Total signature length (smlen) = %u bytes.\r\n", (unsigned int)smlen);
    ...
    if (hw_verify_result != 0) {
        ...
        // [!! 已修正 !!] 將 %llu 改為 %u，並將 (unsigned long long) 強制轉換為 (unsigned int)
        xil_printf("  Recovered message length (mlen_out) = %u bytes.\r\n", (unsigned int)mlen_out);
        print_hex("    -> Recovered Message (mout_hw)", mout_hw, mlen_out);

        // [!! 邏輯調整 !!] 將 Step 5 移至 Step 4 成功的分支內，確保只在驗證成功時才比較
        xil_printf("\r\n[Step 5: Final Check]\r\n");
